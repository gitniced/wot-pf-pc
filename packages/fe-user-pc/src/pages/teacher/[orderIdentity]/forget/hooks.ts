...
export default class forgetHooks {
     ...
     */
    forgetHandler = async (form: any, skipSm2?: boolean) => {
        if (!this.done) {
            this.done = true
            ...
            tempForgetParam = {
                ...
                sid: 1,
                verifyCode,
                type: USER_LOGIN_TYPE.PERSON_TEACHER,
                ...sm2Result,
            }
                ...
                .then(res => {
                    ...
                    if (!success) {
                        if (messageCode === SM2_ERROR_CODE && !skipSm2) {
                            // 加密错误 使用明文登录
                            this.forgetHandler(form, true)
                            return
                        }
                        message.error(res.message)
                    } else {
                        const personMerchantRoute = getCookie('PERSON_MERCHANT_ROUTE')
                        history.push(`/teacher/${personMerchantRoute}/login${location.search}`)
                    }
                })
                .catch(() => {
                    this.done = false
    ...
    goPage = (str: string) => {
        ...
        pathname = pathname.replace('forget', str)
        getParamsHistory.push(pathname)
