/* eslint-disable no-useless-escape */
import { inject, Observer, useLocalObservable } from 'mobx-react'
import styles from './index.module.less'
import bindHooks from './hooks'
import CustomTitle from '@/components/Global/CustomTitle'
import { useEffect } from 'react'
import { Button, Col, Form, Input, Steps, Row } from 'antd'
import type { IRoute } from 'umi'
import type { PageProps } from '@/types'
import MyBreadcrumbs from '@/components/Global/MyBreadcrumbs'
import { getDecodeInfo, TCaptcha } from '@wotu/wotu-components'
const { Step } = Steps

const Email = (props: PageProps) => {
    let { userStore } = props
    const hooks = useLocalObservable(() => new bindHooks())
    const itemLayout = {
        labelCol: { span: 4 },
        wrapperCol: { span: 18 },
    }

    const [oldForm] = Form.useForm()
    const [newForm] = Form.useForm()
    const [form] = Form.useForm()

    useEffect(() => {
        const { email } = userStore?.userData || {}
        oldForm.setFieldsValue({
            account: getDecodeInfo(email || ''),
        })
    }, [])

    const getCodeHandler = () => {
        if (userStore?.userData?.email) {
            if (hooks.stepIndex === 0) {
                oldForm
                    .validateFields(['account'])
                    .then(async res => {
                        const { account } = res
                        hooks.getCode(account, 'old')
                    })
                    .catch(() => {})
            } else {
                newForm
                    .validateFields(['account'])
                    .then(async res => {
                        const { account } = res
                        hooks.getCode(account, 'new')
                    })
                    .catch(() => {})
            }
        } else {
            form.validateFields(['account'])
                .then(async res => {
                    const { account } = res
                    hooks.getCode(account, 'create')
                })
                .catch(() => {})
        }
    }

    return (
        <Observer>
            {() => {
                return (
                    <div className={styles.page}>
                        <MyBreadcrumbs />
                        <div className={styles.content}>
                            <CustomTitle title="邮箱地址" />
                            {userStore?.userData?.email ? (
                                <div className={styles.form}>
                                    <Steps className={styles.steps} current={hooks.stepIndex}>
                                        <Step
                                            title="旧邮箱地址"
                                            description="用旧邮箱地址进行身份验证"
                                        />
                                        <Step title="新邮箱绑定" description="输入新邮箱地址绑定" />
                                    </Steps>
                                    <Row>
                                        <Col span={12} offset={6}>
                                            {hooks.stepIndex === 0 ? (
                                                <Form
                                                    className={styles.form}
                                                    form={oldForm}
                                                    name="bindEmail"
                                                    onFinish={e => {
                                                        hooks.verifyOld(e)
                                                    }}
                                                >
                                                    <Form.Item {...itemLayout} label="旧邮箱地址">
                                                        <Form.Item name="account" noStyle>
                                                            <Input
                                                                className={styles.input}
                                                                disabled={Boolean(
                                                                    userStore?.userData?.email,
                                                                )}
                                                                placeholder="请输入旧邮箱地址"
                                                            />
                                                        </Form.Item>
                                                    </Form.Item>
                                                    <Form.Item {...itemLayout} label="验证码">
                                                        <Row>
                                                            <Form.Item
                                                                name="verifyCode"
                                                                rules={[
                                                                    {
                                                                        required: true,
                                                                        message: '请输入验证码',
                                                                    },
                                                                ]}
                                                                noStyle
                                                            >
                                                                <Input
                                                                    className={styles.code}
                                                                    placeholder="请输入验证码"
                                                                />
                                                            </Form.Item>
                                                            <Form.Item noStyle>
                                                                <TCaptcha
                                                                    depend={{
                                                                        form: oldForm,
                                                                        key: 'account',
                                                                    }}
                                                                    serverVerify={
                                                                        hooks.serverVerify
                                                                    }
                                                                >
                                                                    <Button
                                                                        className={styles.code_btn}
                                                                        disabled={
                                                                            hooks.oldCodeBtnStr !==
                                                                            '发送验证码'
                                                                                ? true
                                                                                : false
                                                                        }
                                                                        onClick={getCodeHandler}
                                                                    >
                                                                        {hooks.oldCodeBtnStr}
                                                                    </Button>
                                                                </TCaptcha>
                                                            </Form.Item>
                                                        </Row>
                                                    </Form.Item>

                                                    <Form.Item noStyle>
                                                        <div className={styles.form_btn}>
                                                            <Button
                                                                loading={hooks.oldBtnLoading}
                                                                type="primary"
                                                                htmlType="submit"
                                                            >
                                                                确定
                                                            </Button>
                                                        </div>
                                                    </Form.Item>
                                                </Form>
                                            ) : null}
                                            {hooks.stepIndex === 1 ? (
                                                <Form
                                                    className={styles.form}
                                                    form={newForm}
                                                    name="bindMobile"
                                                    onFinish={e => {
                                                        hooks.verifyNew(e, userStore?.getUserData)
                                                    }}
                                                >
                                                    <Form.Item {...itemLayout} label="新邮箱地址">
                                                        <Form.Item
                                                            name="account"
                                                            noStyle
                                                            rules={[
                                                                {
                                                                    required: true,
                                                                    message: '请输入新邮箱地址',
                                                                },
                                                                {
                                                                    type: 'email',
                                                                    message: '邮箱地址格式错误',
                                                                },
                                                            ]}
                                                        >
                                                            <Input
                                                                className={styles.input}
                                                                placeholder="请输入新邮箱地址"
                                                            />
                                                        </Form.Item>
                                                    </Form.Item>
                                                    <Form.Item {...itemLayout} label="验证码">
                                                        <Row>
                                                            <Form.Item
                                                                name="verifyCode"
                                                                rules={[
                                                                    {
                                                                        required: true,
                                                                        message: '请输入验证码',
                                                                    },
                                                                ]}
                                                                noStyle
                                                            >
                                                                <Input
                                                                    className={styles.code}
                                                                    placeholder="请输入验证码"
                                                                />
                                                            </Form.Item>
                                                            <Form.Item noStyle>
                                                                <TCaptcha
                                                                    depend={{
                                                                        form: newForm,
                                                                        key: 'account',
                                                                    }}
                                                                    serverVerify={
                                                                        hooks.serverVerify
                                                                    }
                                                                >
                                                                    <Button
                                                                        className={styles.code_btn}
                                                                        disabled={
                                                                            hooks.newCodeBtnStr !==
                                                                            '发送验证码'
                                                                                ? true
                                                                                : false
                                                                        }
                                                                        onClick={getCodeHandler}
                                                                    >
                                                                        {hooks.newCodeBtnStr}
                                                                    </Button>
                                                                </TCaptcha>
                                                            </Form.Item>
                                                        </Row>
                                                    </Form.Item>

                                                    <Form.Item noStyle>
                                                        <div className={styles.form_btn}>
                                                            <Button
                                                                type="primary"
                                                                htmlType="submit"
                                                                loading={hooks.newBtnLoading}
                                                            >
                                                                确认
                                                            </Button>
                                                        </div>
                                                    </Form.Item>
                                                </Form>
                                            ) : null}
                                        </Col>
                                    </Row>
                                </div>
                            ) : (
                                <div className={styles.form}>
                                    <Row>
                                        <Col span={12} offset={6}>
                                            <Form
                                                className={styles.form}
                                                form={form}
                                                name="bindEmail"
                                                onFinish={e => {
                                                    hooks.bindEmail(e, form, userStore?.getUserData)
                                                }}
                                            >
                                                <Form.Item {...itemLayout} label="邮箱地址">
                                                    <Form.Item
                                                        name="account"
                                                        rules={[
                                                            {
                                                                required: true,
                                                                message: '请输入邮箱地址',
                                                            },
                                                            {
                                                                type: 'email',
                                                                message: '邮箱地址格式错误',
                                                            },
                                                        ]}
                                                        noStyle
                                                    >
                                                        <Input
                                                            className={styles.input}
                                                            placeholder="请输入邮箱地址"
                                                        />
                                                    </Form.Item>
                                                </Form.Item>
                                                <Form.Item {...itemLayout} label="验证码">
                                                    <Row>
                                                        <Form.Item
                                                            name="verifyCode"
                                                            rules={[
                                                                {
                                                                    required: true,
                                                                    message: '请输入验证码',
                                                                },
                                                            ]}
                                                            noStyle
                                                        >
                                                            <Input
                                                                className={styles.code}
                                                                placeholder="请输入验证码"
                                                            />
                                                        </Form.Item>
                                                        <Form.Item noStyle>
                                                            <TCaptcha
                                                                depend={{
                                                                    form: form,
                                                                    key: 'account',
                                                                }}
                                                                serverVerify={hooks.serverVerify}
                                                            >
                                                                <Button
                                                                    className={styles.code_btn}
                                                                    disabled={
                                                                        hooks.createCodeBtnStr !==
                                                                        '发送验证码'
                                                                            ? true
                                                                            : false
                                                                    }
                                                                    onClick={getCodeHandler}
                                                                >
                                                                    {hooks.createCodeBtnStr}
                                                                </Button>
                                                            </TCaptcha>
                                                        </Form.Item>
                                                    </Row>
                                                </Form.Item>

                                                <Form.Item noStyle>
                                                    <div className={styles.form_btn}>
                                                        <Button
                                                            type="primary"
                                                            htmlType="submit"
                                                            loading={hooks.btnLoading}
                                                        >
                                                            确认
                                                        </Button>
                                                    </div>
                                                </Form.Item>
                                            </Form>
                                        </Col>
                                    </Row>
                                </div>
                            )}
                        </div>
                    </div>
                )
            }}
        </Observer>
    )
}
const ObserverEmail: IRoute = inject('userStore')(Email)

ObserverEmail.title = '邮箱绑定'

export default ObserverEmail
