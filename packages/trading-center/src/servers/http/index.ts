...
import { getCookie, getLocalStorage, clearCookie, delCookie, getSessionStorage } from '@/storage'
    ...
    res => {
        ...
        if (status === 200) {
            ...
            if (data instanceof Blob) {
                // 文件类型如果是application/json，说明返回的不是正常的文件流，需要弹出后端给的message
                if (data.type === 'application/json') {
                    let failMsg
                    const reader = new FileReader()
                    reader.onload = () => {
                        const failResult = JSON.parse(reader.result as string)
                        failMsg = failResult.message
        ...
        switch (status) {
            ...
            case 401:
                ...
                if (window) {
                    ...
                    const { sid = '' } = sData || {}
                    const pcDomain =
                        findSiteData(sData || {}, 'pcDomain', { findKey: 'baseInfo' }) || ''
                    if (sid.toString() === '1') {
                        location.replace(`/seller/login`)
                    } else {
                        location.replace(`${pcDomain}/account/user/login`)
                    }
                }
                errResponse = '请先登录'
                break
