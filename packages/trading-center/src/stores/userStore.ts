...
export default class UserStore {
    ...
    /**当前组织详情 */
    public selectedOrganizationDetail: any = {} as any

    constructor() {
        makeAutoObservable(this, {}, { autoBind: true })
        makePersistable(this, {
            name: 'transactionUserStore', // 保存的name，用于在storage中的名称标识，只要不和storage中其他名称重复就可以
            properties: [
                'tag',
                'masterHistory',
                'userData',
    ...
    updatePortalData = (tempPortalData: PortalData) => {
        this.portalData = JSON.parse(JSON.stringify(tempPortalData))
    }
    ...
    getOrganizationData = async () => {
        if (getCookie('SELECT_ORG_CODE')) {
            let organizationData = await http(
                `${globalApi.getOrgDetail}/${getCookie('SELECT_ORG_CODE')}`,
                'get',
                {},
                {
                    repeatFilter: false,
            ...
            this.selectedOrganizationDetail = organizationData
        }
    }
