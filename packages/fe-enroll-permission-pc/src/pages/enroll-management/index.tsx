...
import { findSiteData, usePageListConfig } from '@wotu/wotu-components'
...
import FollowModal from '../student-pool/components/FollowModal'
...
const EnrollManagement = (props: IRoute) => {
     ...
     *  默认的活动code
     */
    const [defaultCode, setDefaultCode] = useState<string>(enrollCode || '')
    ...
    // 报名缴费
    const enable_apply_pay = findSiteData(siteData, 'enable_apply_pay', {
        findKey: 'configList',
    })?.value
    /**
     * tab 列表
    ...
    // 学员跟进
    const handleFollow = (rowData: any) => {
    ...
    return (
        <div className={styles.enroll_management}>
            <div className={styles.main}>
                <CustomTitle title="报名管理" marginBottom={0} />
                <BusinessTable
                    rowKey={'code'}
                    // @ts-ignore
            ...
            <FollowModal open={!!record} data={record} onOk={handleOk} onCancel={handleCancel} />
        </div>
    )
}
const ObserverPassword: IRoute = observer(EnrollManagement)

ObserverPassword.title = '报名管理'
