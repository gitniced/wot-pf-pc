...
import { SIGN_STATUS_OPTIONS, SIGN_STATUS_TEXT } from '../student-pool/consts'
...
export function enrollMananementColumns(
    ...
    return [
        ...
        },
        {
            ...
            width: 200,
            formOrder: 2,
            render: (userIdentify: string, record: any) => {
                return (
                    <div>
                        {record.isHideMsg
                            ? getDecodeInfo((userIdentify as unknown as string) || '', '4')
                            : getDecodeInfo((userIdentify as unknown as string) || '')}
                    </div>
                )
            },
        },
        ...
        },
        {
            ...
            render: (_, records: any) => {
                return (
                    <Tooltip title={records?.activityName}>
                        <a
                            onClick={() =>
                                window.open(
                                    `${window.location.origin}/enroll-center/event-management/details?code=${records?.activityCode}`,
                                )
                            }
                        >
        ...
        {
            search: batch_registration !== '1',
            title: '报名方式',
            key: 'method',
            dataIndex: 'method',
            width: 120,
            hide: batch_registration !== '1',
            render: (_, records: any) => {
        ...
        },
        {
            search: true,
            title: '学员跟进',
            dataIndex: 'userPoolStatus',
            fixed: 'right',
            formOrder: 9,
            width: 120,
            render: (status, { remark }: any) => {
                return (
                    <Space>
                ...
                return (
                    <Space>
                        ...
                        {records.type !== EVENT_KIND_ENUM.ORGANIZATION && (
                            <NavLink to={`/enroll-management/audit?code=${records.code}&type=info`}>
                                详情
                            </NavLink>
                        )}
                        {/* 机构没有审核 status 1待审核 */}
                        {records.type !== EVENT_KIND_ENUM.ORGANIZATION &&
                            records.status === AUDIT_STATUS_ENUM.WAIT && (
                                <NavLink to={`/enroll-management/audit?code=${records.code}`}>
                                    审核
                                </NavLink>
                            )}
                        ...
                        */}
                        ...
                        {AUDIT_STATUS_ENUM.SUCCESS === records.status && (
                            <a onClick={() => handleFollow(records)}>学员跟进</a>
                        )}
                    </Space>
                )
            },
