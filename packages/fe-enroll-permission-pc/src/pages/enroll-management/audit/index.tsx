...
import { Button, Image, Descriptions, Tooltip, Tag } from 'antd'
// import { ManOutlined, WomanOutlined } from '@ant-design/icons'
import dayjs from 'dayjs'
import type { ENROLL_TABLE_LIST_ENUM, INFO_ENUM } from './const'
import { SEX_TEXT, STATUS_ACTIVE, statusActive } from './const'
import {
    // STATUS_AUDIT,
    // STATUS_AUDIT_TEXT,
    AUDIT_STATUS_ENUM,
...
import { REGISTRATION_METHOD_ENUM } from '../const'
...
const Audit = () => {
    ...
    const translateValue = (item: any, flag = false) => {
        ...
        const NEED_DESENSITIZATION_LIST = ['NAME', 'PHONE_NUMBER', 'ID_NUMBER', 'ID_NUMBER_COMMON']
    ...
    const getAliasValue = (orderAlias: keyof typeof INFO_ENUM) => {
        ...
        switch (orderAlias) {
            case 'NAME':
                ...
                const finallyName = matchItem?.value ? matchItem?.value : ''
                return <DesensitizationItem preStr={finallyName} type="1" />
            ...
            case 'GENDER':
                ...
                return matchItem?.value
                    ? genderStr[matchItem?.value.toString()]
                    : gender
    ...
    const getEnrollTableListItem = (type: ENROLL_TABLE_LIST_ENUM, list: any[]) => {
        return (
            <div className={styles.enroll_table_item}>
                ...
                <Minititle
                    title={ENROLL_TABLE_LIST_ENUM_NAME[type]}
                    titleStyles={{
                        marginTop: '24px',
                        fontSize: '16px',
                        fontWeight: '500',
    ...
    return (
        <div className={styles.audit_page} ref={pageRef}>
            <div className={styles.audit_title}>
                <CustomTitle title={urlType !== 'info' ? '审核' : '详情'} />
                {urlType !== 'info' && store.auditDetail?.status === AUDIT_STATUS_ENUM.WAIT && (
            ...
            <div className={styles.enroll_info_wrapper}>
                <div>
                    <span>在线报名：</span>
                    <span
                        className={styles.enroll_name}
                        onClick={() => {
                            window.open(
                                `${window.location.origin}/enroll-center/event-management/details?code=${store.auditDetail?.applyCode}`,
                            )
                        }}
                    >
                        {store.auditDetail?.name}
                    </span>
                </div>

                <div>
                    <span>报名状态：</span>
                    {store.auditDetail?.applyStatus ? (
                        <Tag
                            className={`${styles.enroll_status} ${
                                styles[`enroll_status_${store.auditDetail?.applyStatus}`]
                            }`}
                            color={statusActive[store.auditDetail?.applyStatus]}
                        >
                            {STATUS_ACTIVE[store.auditDetail?.applyStatus]}
                        </Tag>
                    ) : (
                        '-'
                    )}
