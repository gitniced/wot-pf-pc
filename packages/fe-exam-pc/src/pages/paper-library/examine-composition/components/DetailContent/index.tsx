...
const DetailContent = (props: DetailContentProps) => {
    ...
    return (
        <div className={styles.detail_content}>
            ...
            <div className={styles.description_item}>
                <label className={[styles.required, styles.no_shrink].join(' ')}>
                    难易程度分布：
                </label>
                <div className={styles.grow}>
                    <span className={styles.info}>{difficultyLimit ? '限制' : '不限制'}</span>
                    {difficultyLimit ? (
                        <>
                            <Table
                                bordered
                                rowKey="level"
                                className={styles.table}
                                columns={getLevelColumns(isEdit)}
                                dataSource={difficultyTableData}
                                pagination={false}
                                summary={(pageData: any) => (
                                    <LevelSummary
                                        type={'examine_create_detail'}
                                        questionTableData={[]}
                                        pageData={pageData}
                                    />
                                )}
                            />
                            {isEdit &&
                                lackLevelType(difficultyTableData as unknown as any).length > 0 && (
                                    <span className={styles.table_extra}>
                                        难易程度为
                                        {lackLevelType(difficultyTableData as unknown as any).join(
                                            '、',
                                        )}
                                        的题目数量不足，请先前往题库添加
                                    </span>
                ...
                <div className={styles.description_item}>
                    ...
                    <div>
                        ...
                        <p className={styles.describe}>
                            以上勾选题型至少抽取{questionTypeLeast}种，总题数{questionTotal}题
                        </p>
                    </div>
                </div>
            )}
                ...
                <div>
                    <p className={styles.info}>
                        ...
                        {randomQuestionState ? (
                            randomQuestionType === 0 ?
                                <span>
                                    ，从上方题型结构中随机抽取 {randomQuestionNum} 题
                                </span>
                                :
                                <span>
                                    ，从 {randomQuestionNum} 题中，随机按上方题型结构抽取
                                </span>
                        ) : (
