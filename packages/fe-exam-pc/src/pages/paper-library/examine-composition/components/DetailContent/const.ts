...
import type { LevelItem } from '../../edit/interface'
...
export const compositionType: Record<string, { value: string; describe: string }> = {
    authenticate: {
        value: '按考评点组卷',
        describe: '按照要素细目表设置的权重进行抽题组卷，所抽试题均为绑定了考评点的试题',
    },
    questiontype: {
        value: '按题型组卷',
 ...
 * @returns
 */
export const lackLevelType = (data: LevelItem[] = []) => {
    const typeArr: any = []
    data.forEach(item => {
        const { needNumber, count, level: type } = item || {}
        if (needNumber > count) {
            typeArr.push(`【${DIFFICULTY_LEVEL[type]}】`)
        }
    })
    return typeArr
}
