...
import { getFrontPointCount } from './api'
...
const getQuestionTableList = async (values: any) => {
    ...
    const res = ((await getFrontPointCount(params)) as unknown) || []
    return res
}
...
class handleValues {
    ...
    public processedValue(
        ...
        /** 处理附件 */
        const fileTypeList = ['examinationCommitmentLetter', 'expertReviewMaterials'] 
        if(fileTypeList.includes(fieldName)){
            const newData: Record<string, any> =  {}
            for(let key in fieldValue){
                newData[key] = fieldValue[key].map((item: any) => item.url)?.[0]
            }
            return newData
        }else if (!(this as any)[fieldName]) {
            return fieldValue
        } else return (this as any)[fieldName](fieldValue, values, form, type, lastOrganizationCode, commonParams)
    }
}
