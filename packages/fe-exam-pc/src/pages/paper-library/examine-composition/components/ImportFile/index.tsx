...
import { IMPORT_TEMPLATE } from '@/components/BatchImport/Before'
...
const PaddingModal = (props: ChildrenProps) => {
    ...
    const getImportData = async (code: string) => {
        const res =
            ((await http(`${API.excelResult}/${code}`, 'get', {
                ...commonParams,
            })) as unknown as any) || {}
        ...
        <div className={styles.padding_modal}>
            <div className={styles.padding_content}>
                ...
                <div className={styles.template}>
                    ...
                    <Button onClick={() => window.open(IMPORT_TEMPLATE)}>
                        <Space>
                            <DownloadOutlined />
                            <span>下载模板</span>
                        </Space>
...
const LoadingModal = (props: ChildrenProps) => {
    ...
    const openUserPCPage = () => {
        window.open(`${siteStore?.siteData?.data?.baseInfo?.pcDomain}/account/organization/action`)
    }
    return (
