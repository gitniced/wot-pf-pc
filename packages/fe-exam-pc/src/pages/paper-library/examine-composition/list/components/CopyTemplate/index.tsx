...
import API from './api'
import type { CopyTemplateProps } from './interface'
import styles from './index.module.less'
import { generateName } from './const'
...
const CopyTemplate = (props: CopyTemplateProps) => {
    ...
    const OnFinish = async () => {
        ...
        let businessName = `${work?.label}`
        if (type) {
            businessName = `${businessName}/${type.label}`
        }
        if (level) {
            businessName = `${businessName}/${level.label}`
        }
