...
const transToSubmitData = (values: Record<string, any>, type: string) => {
    ...
    const obj: Record<string, any> = {
        ...
        randomQuestionState: randomQuestionState ? 1 : 0,
    ...
    if (randomQuestionState) {
        obj.randomQuestionNum =
            values[`randomQuestionNum_${randomQuestionType}`] || randomQuestionNum
    }
    ...
    if (difficultyLimit) {
        if (type !== 'template') {
            obj.difficultyConfigList = difficultyTableData
                ?.filter((item: any) => item?.needNumber > 0)
                ?.map((item: any) => {
                    return {
                        level: item?.level,
            ...
            obj.difficultyConfigList = templateDifficultyTableData
                ?.filter((item: any) => item?.needNumber > 0)
                ?.map((item: any) => {
                    return {
                        level: item?.level,
    ...
    delete resultData.difficultyTableData
    delete resultData.templateDifficultyTableData
    return resultData
}
/**
 * 转换为表单值
...
const transToFormFieldsValue = (values: Record<string, any>) => {
    ...
    const obj: Record<string, any> = {
        ...
        randomQuestionType,
        [`randomQuestionNum_${randomQuestionType}`]: randomQuestionNum,
    }
