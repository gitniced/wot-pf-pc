...
const ExamDetailComp = (data: Partial<ExamDetail>) => {
    ...
    const renderToolTip = (title?: string) => {
        return <Tooltip placement='topLeft' title={title} overlayClassName={styles.exam_detail_tooltip}>{title}</Tooltip>
    }
    const dataList = [
        {
            label: '职业/工种/等级：',
        ...
        {
            label: '考试时间：',
            value: renderToolTip(`${dayjs(data?.startTime).format(FORMAT)} 至 ${dayjs(data?.endTime).format(FORMAT)}`)
        },
        {
            label: '考场：',
        ...
        {
            label: '签到方式：',
            value:  renderToolTip(SIGN_TYPE_TEXT[data?.needSign || 0])
        },
        {
            label: '考试状态：',
            value:  renderToolTip(EXAM_STATE_TEXT[data?.examState || 0])
        }
    ]
    return (
