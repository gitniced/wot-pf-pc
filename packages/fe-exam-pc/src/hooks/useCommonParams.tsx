...
const useCommonParams = () => {
    ...
    let defaultBelongType: any = {}
    userType === 'org' ? (defaultBelongType = BELONG_TYPE_ENUM.ORGANIZE) : ''
    userType === 'merchant' ? (defaultBelongType = BELONG_TYPE_ENUM.MERCHANT) : ''
    if (!type) return { belongType: defaultBelongType }
    const { subject, skill } = getSubjectAndSkill(type)
    return {
        skill,
        subject,
        belongType: defaultBelongType,
    }
}
