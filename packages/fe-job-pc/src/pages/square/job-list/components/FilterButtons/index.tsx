...
const CompanyIndustry: React.FC<ButtonProps> = ({ value = [], onChange, getOptions, options }) => {
    ...
    const onClick = (id: number) => {
        const index = value.findIndex((item: number | string) => item === id)
        if (index !== -1) {
...
/** 职位类型 */
const PositionType: React.FC<ButtonProps> = ({
    value = [],
    onChange,
    getOptions,
    options,
...
const FilterButtons: React.FC<{
    ...
    useEffect(() => {
        form.setFieldsValue(jobParams)
    }, [jobParams])

    /** 重置 */
    const onReset = () => {
        form.resetFields()
        form.validateFields().then(val => setJobParams({ ...val, pageNo: 1 }))
    }
    ...
    return (
        <div className={styles.filter_buttons}>
            <Form
                layout="inline"
                form={form}
                onValuesChange={changedValues => setJobParams({ ...changedValues, pageNo: 1 })}
            >
                ...
                <Form.Item name="recruitTypes">
                    <Selection
                        title="招聘类型"
                        type="checkbox"
                        options={store.recruit}
                        getOptions={store.getRecruit}
                    />
                </Form.Item>
            </Form>
