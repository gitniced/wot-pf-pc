...
const AnswerDetailModal = ({
    ...
    const projectClassificationList = ['就业心态', '就业条件', '求职能力']
    ...
    if ((Number(resultValue) < 1.5 && index === 2) || (Number(resultValue) > 30 && index === 1)) {
        ...
        (Number(resultValue) > 2 && index === 0) ||
        (Number(resultValue) > 2.5 && index === 2) ||
        (Number(resultValue) > 70 && index === 1)
    ) {
        level = 3
    }
    ...
    return (
        ...
        >
            ...
            <div
                className={`${styles.result} ${styles[`result_${2}`]} ${
                    styles[`level_${typelevel}`]
                }`}
            >
                结果：{type === 1 ? resultValue : LEVEL_TEXT[resultValue!]}
            </div>
                    ...
                    <div className={styles.question_item} key={item.questionCode}>
                        ...
                        <div className={styles.option_list}>
                            {item.options?.map(option => (
                                <Radio
                                    checked={option.sequence === store.answerObj[item.code!]}
                                    key={option.sequence}
                                >
