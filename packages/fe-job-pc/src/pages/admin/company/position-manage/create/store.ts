...
import type { TypeOption } from './interface'

const sid = getLocalStorage('SID')

class PositionManageAddStore {
    // 招聘单位下拉列表
    public recruitCompanyList = []
    public recruitCompanyTotalCount = 0
    public recruitCompanyQuery = { pageNo: 1, pageSize: 10 }
    // 教育经历下拉列表
    public educationOption: TypeOption[] = []
    // 经验下拉列表
    public experienceOption: TypeOption[] = []
    // 薪酬范围下拉列表
    public salaryTypeOption: TypeOption[] = []
    // 结算方式下拉列表
    public settlementTypeOption: TypeOption[] = []
    // 一级岗位
    ...
    recruitCompany = async (
        params: { pageNo: number; pageSize: number } = { pageNo: 1, pageSize: 10 },
    ) => {
        const resp: any = await Http(api.recruitment_page, 'POST', params)
        const { data, totalCount, currentPage } = resp

        this.recruitCompanyList = currentPage === 1 ? data : [...this.recruitCompanyList, ...data]
        this.recruitCompanyTotalCount = totalCount
        this.recruitCompanyQuery = params
    }
    ...
    jobTree = async (id: number) => {
        const _resp = await Http(`${api.list_tree}/${id}`, 'get', {}, { repeatFilter: false })
    }
