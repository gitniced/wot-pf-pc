...
import api from './api'
import { EmploymentAssistanceRecordDetailDto, AssistanceFromQuestionDto, QuestionAndAnswerDto } from './interface'
...
class Store {
    ...
    getListSystemMeasure = () => {
        http(`${api.listSystemMeasure}`, 'GET', {}).then((res: any) => {
            const measureList = res?.map(item => {
                return {
                    title: item,
    ...
    getAssistanceDetail = async (code: string) => {
        const select_org = getCookie('SELECT_ORG_CODE')
        return http(`${api.getAssistanceDetail}/${select_org}`, 'GET', { recordCode: code }).then((res: EmploymentAssistanceRecordDetailDto) => {
