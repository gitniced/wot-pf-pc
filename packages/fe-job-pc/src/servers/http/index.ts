...
import { delCookie, setCookie, getCookie, getLocalStorage } from '@/storage'
...
Http.interceptors.request.use(
    (config: RequestConfig) => {
        ...
        // config.headers.Authorization = "eyJhbGciOiJIUzM4NCJ9.eyJqdGkiOiJqd3RfaWQiLCJzdWIiOiIxNjkxNjgzMUIyVlJERVlPIiwiaWF0IjoxNjk0OTk5MzUzLCJhdWQiOiJXRUIiLCJleHAiOjE2OTc1OTEzNTMsInNpZCI6MTAwMSwidHlwZSI6IlVTRVIiLCJncm91cFR5cGUiOjJ9.NELOI_0sCBS0dzx__12rOzJMDUm_BjPmF3JcnEV8uCVO_DmNMIHqiRI2z8h2rEav"
    ...
    res => {
        ...
        if (status === 200) {
            ...
            if (config.form) {
                ...
                if (success === true) {
                    if (Reflect.has(config, 'showMessage')) {
                        // @ts-ignore
                        message.success(config?.showMessage || requestMessage)
                    }
        ...
        switch (status) {
            ...
            case 401:
                ...
                if (window) {
                    ...
                    let loginUrl = RUN_ENV === 'local' ? '' : pcDomain
                    ...
                    if (portalCode) {
                        RUN_ENV === 'local'
                            ? window.location.replace(
                                  `http://localhost:8031/${portalCode}/user-center/user/login`,
                              )
                            : window.location.replace(`/${portalCode}/user-center/user/login`)
                    } else {
                        RUN_ENV === 'local'
...
export default <T, K>(
    ...
    return Http.request<RequestConfig, AxiosResponse<K>>({
        ...config,
        url,
        method,
