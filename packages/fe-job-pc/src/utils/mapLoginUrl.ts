...
export const LOGIN_BACK_MAP: Record<string, string[]> = {
    ...
    /**
     * 资源方用户中心pc
     * 对应 资源方pc {merchantDomain} 资源方中台pc {merchantMidDomain} 课程资源方pc {courseMerchantDomain} 题库资源方pc {questionMerchantDomain}
     */
    merchantUserDomain: [
        'merchantDomain',
        'merchantMidDomain',
        'courseMerchantDomain',
        'questionMerchantDomain',
    ],
    /**
     * 讲师用户中心pc
...
export const getMapLoginUrl = (params: GetMapLoginUrlType = { type: 'storage' }): string => {
    ...
    Object.keys(baseInfo).find(tempKey => {
        if (baseInfo[tempKey] === getDomain()) {
            originMatchKey = tempKey
            return true
        } else {
            return false
        }
    ...
    if (originMatchKey) {
        if (SELF_LOGIN_MAP[originMatchKey]) {
            ...
            Object.keys(LOGIN_BACK_MAP).find((tempKey: string) => {
                if (LOGIN_BACK_MAP[tempKey].includes(originMatchKey)) {
                    matchUserKey = tempKey
                    return true
                } else {
                    return false
                }
