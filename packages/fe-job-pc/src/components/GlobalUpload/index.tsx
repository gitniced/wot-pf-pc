...
const accetpMap: Record<string, string> = {
...
interface FileProps {
    ...
    accept?:
        ...
        | 'ppt'
        | 'mp4'
        | Partial<'zip' | 'excel' | 'pdf' | 'image' | 'word' | 'csv' | 'ppt' | 'mp4'>[]
    // 是否开启拖拽
    drag?: boolean
    // upload样式
    className?: string
    ...
    // 	业务类型 1导入组织成员 2用户头像 3合同附件 4客户附件 5站点图片 6组织认证图片 7 二要素认证图片 8商品图片 9组织头像 10推荐位上传 11财务凭证 ,12菜单icon,18图文附件
...
export default function GlobalUpload({
    ...
    // 数量限制
    const checkLimit = (fileList: any) => {
        return new Promise((resolve, reject) => {
            // @ts-ignore
            const isLen = (uploadRef?.current?.fileList?.length ?? 0) + fileList.length
            amount && isLen > amount ? reject(Error('')) : resolve(true)
        }).then(
            () => {
                return true
            },
    ...
    const checkSize = (file: { size: number }) => {
        return new Promise((resolve, reject) => {
            size && file.size > Math.pow(1024, 2) * size ? reject(Error('')) : resolve(true)
        }).then(
            () => {
                return true
            },
    ...
    // 单张传图时计算图片宽高 适应显示
    const previewImageLoaded = (e: any) => {
        // @ts-ignore
    ...
    return (
        <div className={styles.global_upload} style={otherProps?.style || {}}>
            {isDragger()}
            <Modal
                centered
                open={previewVisible}
