...
export default class AreaStore {
    ...
    getLoad = async (selectedOptions: AreaOption[]) => {
        ...
        let res: any = await http(api.city, 'get', { parentCode: code })
    ...
    // 获取城市列表
    getAreaList = async (parentCode: string | number, nationwideState?: number) => {
        let res: any = await http(api.city, 'get', { parentCode, nationwideState })
    ...
    getAreaLeaf = async (parentCode: string | number) => {
        let res: any = await http(api.city, 'get', { parentCode })
        let result: AreaOption[] = []
        res.map((item: any) => {
            result.push({
                value: item.code,
    ...
    initDefaultValue = async (value: string[]) => {
        if (Array.isArray(value)) {
            ...
            data = data.map(areaItem => {
                if (areaItem.value === String(provinceCode)) {
                    areaItem.children = cityData?.map(v => ({ ...v, isLeaf: this.type === 'city' }))
                }
                return areaItem
            })
            runInAction(() => {
