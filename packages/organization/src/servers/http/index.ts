...
import { getCookie, getLocalStorage, clearCookie, setCookie } from '@/storage'
...
Http.interceptors.response.use(
    ...
    error => {
        ...
        switch (status) {
            ...
            case 401:
                const typeEnum = { user: 1, org: 2, merchant: 3 }
                ...
                if (window) {
                    ...
                    const { sid = '', baseInfo = {} } = sData || {}
                    let { loginUrl, merchantUserDomain } = baseInfo || {}
                    let sourceType = getLocalStorage('SOURCE_TYPE')
                    // 清除store的数据
                    window?.self_store?.userStore?.initStore?.()
                    if (sid.toString() === '1') {
                        // 资源方  回到对应来源身份的资源方登录页
                        window.location.href = `${merchantUserDomain}/seller/login?sourceType=${sourceType}`
                    } else {
                        // 回到用户中心登录页
