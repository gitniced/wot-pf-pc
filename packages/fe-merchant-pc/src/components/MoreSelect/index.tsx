 ...
 * @params search  默认为true 支持搜索 为false时 不支持搜索
 * @params isShowCodeAddName  是否显示code加+ name   例如   xxx哈哈哈
 */
...
const MoreSelect = (props: MoreSelectProps) => {
    ...
    const defaultOptions = all ? [{ label: '全部', value: '' }] : []
    ...
    const singleDoRequest = (params: any) => {
        const searchKeyWard = params[searchKey!]

        searchStr = searchKeyWard
        // 页码，及搜索值变化时，发请求
        requestUrl &&
            http(
                requestUrl,
                ...
                if (isHasPage) {
                    ...
                    if (all) {
                        tempData = [{ label: '全部', value: '' }].concat(tempData)
                    }
                }
    ...
    }, [paramsUpdate ? requestParams : null])
...
export default forwardRef(MoreSelect)
