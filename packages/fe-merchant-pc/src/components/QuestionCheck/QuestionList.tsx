...
import { Tag, Space, Typography, Modal, Tooltip, message } from 'antd'
...
import { QUESTION_LEVEL_TEXT, QUESTION_TYPE_ENUM, SUBJECT_TYPE_ENUM } from '@/constants'
...
const QuestionList: React.FC<QuestionListProps> = ({
    ...
    const handleLinkToEdit = (code: string) => {
        window.open(`/merchant-center/question/${type}/edit?code=${code}`, '_blank')
    }
                        ...
                        <div className={styles.item_header}>
                            <div className={styles.header_left}>
                                ...
                                {question.distinction && (
                                    <div className={styles.left_item}>
                                        区分度：{question.distinction}
                                    </div>
                                )}
                                {question.authPoint && (
                                    <Tooltip title={question.authPoint}>
                                        <div className={styles.left_item}>
                                            鉴定点：
                                            {ellipsisText(question.authPoint, maxCount)}
                                        </div>
                                    </Tooltip>
                                )}
                            </div>
                            <div className={styles.header_right}>
                                ...
                                {showEdit && (
                                    <Space
                                        ...
                                        onClick={() => {
                                            ...
                                            handleLinkToEdit(question.code)
                                        }}
                                    >
                                        <FormOutlined />
                                ...
                                {!(showEdit && showDelete) &&
                                    (index === 0 ? (
                                        ...
                                        <Typography.Text style={{ color: '#1678FF' }}>
                                            已有试题
                                        </Typography.Text>
                                    ))}
                            </div>
