...
const Index = () => {
    ...
    const [storeShow, setStoreShow] = useState(true)
    const [publicSuccessShow, setPublicSuccessShow] = useState(false)
    ...
    useEffect(() => {
        ...
        if (Reflect.has(query, 'code')) {
            ;(async () => {
                const resp: any = await store.professDetail(query.code)
                ...
                if (resp?.partTimeRecruit) {
                    ...
                    if (resp.partTimeRecruit.workTimeType === 2) {
                        const [hour, minute] = workTimeStart.split(':')
                        const [hour1, minute1] = workTimeEnd.split(':')
        ...
        <div className={styles.position_add_container}>
            ...
            >
                <Spin size="large" spinning={loading}>
                    <div style={{ height: 300, overflowY: 'scroll' }}>{desc}</div>
                </Spin>
            </Modal>
            ...
            {/* <Breadcrumb>
                <Breadcrumb.Item>
                    <a onClick={jumpPositionManage}>职位管理</a>
                </Breadcrumb.Item>
                <Breadcrumb.Item>{query?.code ? '编辑' : '新建'}职位</Breadcrumb.Item>
            </Breadcrumb> */}
            ...
            >
                ...
                >
                    <TxMap store={store} organizationAddress={store.organizationAddress} />
                </Form.Item>
                ...
                {/* <div>
                    职位要求 系统将根据您的要求为您精准匹配合适的求职者{' '}
                    {form.getFieldValue('recruitType')}
                </div> */}
                {/* 社招职位要求 */}
                {recruit === 1 && (
                    <>
                            ...
                            <Row>
                                <Col>
                                    <Form.Item
                                        ...
                                        rules={[{ required: true, message: '请选择最小实习月数' }]}
                                    >
                                        <Select
                                            placeholder="请选择"
                                ...
                                <Col offset={1}>
                                    <Form.Item
                                        ...
                                        rules={[
                                            { required: true, message: '请选择一周最小到岗天数' },
                                        ]}
                                    >
                                        <Select
                                            getPopupContainer={triggerNode =>
                            ...
                            {workTimeType === 2 && (
                                <div style={{ marginTop: 12 }}>
                                    <Form.Item
                                        ...
                                        rules={[{ required: true, message: '请选择自定义时间段' }]}
                                    >
                ...
                <Form.Item {...tailLayout}>
                    <Space>
                        ...
                        <Button
                            type="primary"
                            onClick={debounce(publicFunc, 2000, { leading: true, trailing: false })}
                        >
                            发布
                        </Button>
                    </Space>
