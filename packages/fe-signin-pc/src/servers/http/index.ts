...
import { getCookie, getLocalStorage, delCookie, getSessionStorage, setCookie } from '@/storage'
...
Http.interceptors.request.use(
    (config: RequestConfig) => {
        ...
        var selectOrgCode = getCookie('SELECT_ORG_CODE');
        if (selectOrgCode) {
    ...
    error => {
        ...
        switch (status) {
            ...
            case 401:
                ...
                //     } else {
                //         masterHistoryVO?.masterHistory?.replace('/user/login')
                //     }
                // }
                // errResponse = '请先登录'
                        ...
                        case 'portal':
                            {
                                ...
                                RUN_ENV === 'local'
                                    ? window.location.replace(
                                        `http://localhost:8031/${portalAlias}/user-center/user/login`,
                                    )
                                    : window.location.replace(
                                        `/${portalAlias}/user-center/user/login`,
                                    )
                            }
