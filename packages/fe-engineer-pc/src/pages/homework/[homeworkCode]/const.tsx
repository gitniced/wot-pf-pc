...
const handleView = (record: any, scheduleCode: string, homeworkCode: string) => {
    const { evaluationTask } = record
    if (scheduleCode && homeworkCode) {
        history.push(
            `/homework/${homeworkCode}/info?scheduleCode=${scheduleCode}&evaluationTask=${evaluationTask}&mode=view`,
        )
    }
}

const handleGrade = (record: any, scheduleCode: string, homeworkCode: string) => {
    const { evaluationTask } = record
    if (scheduleCode && homeworkCode) {
        history.push(
            `/homework/${homeworkCode}/info?scheduleCode=${scheduleCode}&evaluationTask=${evaluationTask}&mode=grade`,
        )
    }
}
