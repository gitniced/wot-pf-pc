    ...
    error => {
        ...
        switch (status) {
            ...
            case 401:
                ...
                //         BUILD_ENV === 'local'
                //             ? ''
                //             : findSiteData(siteData, 'loginUrl', { findKey: 'baseInfo' }) || ''
                ...
                //                       `http://localhost:8001/${portalCode}/user-center/user/login`,
                //                   )
                //                 : window.location.replace(`/${portalCode}/user-center/user/login`)
                //             break
                //         case 'workbench':
                //             BUILD_ENV === 'local'
                //                 ? window.location.replace(`http://localhost:9004/user/login`)
                //                 : loginUrl
                //                 ? window.location.replace(`${loginUrl}/user/login`)
...
export default <T, K>(url: string, method: Method, data: T, config?: RequestConfig): Promise<K> => {
    let _url = url
    if (config?.query && typeof config.query === 'object' && Object.keys(config.query).length > 0) {
        const urlSearchParams = new URLSearchParams()
        Object.entries(config.query).forEach(([key, value]) => {
            if (value !== undefined) urlSearchParams.append(key, value)
        })
        if (urlSearchParams.size > 0) _url = `${url}?${urlSearchParams.toString()}`
    }
