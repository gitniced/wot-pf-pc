...
export const getEnrollList = async (templatePreviewList: any, type: string, cds?: string[]) => {
    ...
    const portalDataObj = getLocalStorage('SITE_STORE')?.portalData || {}
    const { organizationLogo, organizationName } = portalDataObj?.[organizationCode] || {}
    const actions: Record<string, any> = {
        default: async () => {
        ...
        custom: async (arr: any) => {
            ...
            return sortRes(arr, res?.map((item: any) => ({...item, organizationLogo, organizationName})))
        },
        by_category: async (arr: any[]) => {
            const TYPE_TAG_TRANSFORMED_NUMBER: Record<string, number> = {
                ORGANIZATION: 1,
                REVIEWS_PLAN: 2,
                TRAINING_PLAN: 3,
                TRAINING_CLASS: 4,
            ...
            return res?.data?.map((item: any) => ({...item, organizationLogo, organizationName})) || []
        },
    }
...
export const getClassList = async () => {
    const data = await Http(
        '/career_main/course/student_class',
        'post',
        { organizationCode },
        { repeatFilter: false },
    )
    ...
    //                 id: 1,
    //                 name: '班级名称xxx',
    //                 startTime: 324234234234,
    //                 endTime: 23123123123123,
    //                 totalPeriod: '50.9',
    //             },
    //             {
    //                 id: 2,
