...
Http.interceptors.request.use(
    (config: RequestConfig) => {
        ...
        // 接口请求增加alias别名
        const SiteStore = JSON.parse(localStorage.getItem('gatewaySiteStore')!) || {}
        ...
        if (!unCookie.includes(currentAlias) && portalCode) {
            config.headers['X-Site-Org'] = portalCode
        }
        ...
        switch (status) {
            ...
            case 401:
                ...
                if (window) {
                    ...
                    history.push(`/${currentDomain}/user-center/user/login`)
                }
                errResponse = '请先登录'
                break
