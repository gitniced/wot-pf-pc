...
import { findSiteData, useCountDown } from '@wotu/wotu-components'
...
import { setRedColor } from '../TimeTips'
...
const EnrollBtn = (props: EnrollFooterProps) => {
    // const jyAliasList = [
    //     'shqznlsx',
    //     'demoqznlsx',
    //     'hnqznlsx',
    ...
    //@ts-ignore
    const { enrollCode, siteStore } = props
    ...
    useEffect(() => {
        if (enrollCode) {
            store.getActivityDetail(enrollCode!)
        }
    }, [enrollCode])
    ...
    const getBindUrl = () => {
        ...
        const loginUrl =
            findSiteData(siteStore?.siteData || {}, 'pcDomain', {
                findKey: 'baseInfo',
            }) || ''
        switch (platform) {
            case 'portal':
    ...
    /**前往认证 */
    const toAudit = () => {
        if (selectUserType === 'org') {
            message.warn('仅支持个人用户操作，请重新登录')
            return
        }
        Modal.warning({
            width: 480,
        ...
        if (isLogin()) {
            if (ndVerify) {
                if (store?.activityDetails?.verify) {
                    setSessionStorage('EVENT_CODE', query.code)
            ...
            // 门户报名
            if (portalCode) {
                setCookie('FROM_URL', `/${currentAlias}/enroll-gateway${enrollUrl}`)
                setSessionStorage('EVENT_CODE', query.code)
                ...
                setSessionStorage('EVENT_CODE', query.code)
                location.href = `${loginUrl}/account`
            }
        }
    }
    const gotoPay = () => {
    ...
    const openMyApply = () => {
        ...
        const padDomain =
            RUN_ENV === 'local'
                ? 'http://localhost:8021'
                : findSiteData(siteData, 'padDomain', { findKey: 'baseInfo' })
        const currentAlias = location.pathname.split('/')?.[1] || ''
        const platform = getSessionStorage('PLATFORM')
        switch (platform) {
            case 'train':
            ...
            case 'workbench':
                ...
                break
            case 'portal':
                window.location.href = `/${currentAlias}/mine/enroll-center/my-enrollment`
                break
            case 'middle':
                if (judgeIsJyAlias(siteAlias)) {
                    window.location.href = `${padDomain}/enroll-center/my-enrollment`
                } else {
                    window.location.href = `${orgDomain}/enroll-center/my-enrollment`
                }
                break
...
export default inject('siteStore', 'userStore')(observer(EnrollBtn))
